#!/usr/bin/env python3
"""Pick a diverse subset of VWA tasks from a `*.raw.json` list.

This is a generalized version of the classifieds picker. It is intended for
quick screening across many branches (feat/integrate) by selecting a smaller,
less redundant set of tasks.

Two-stage idea:
  Stage 1 (mechanical): reduce redundancy by grouping on `intent_template_id`.
  Stage 2 (compare & prune): further remove near-duplicates by intent similarity,
                             and optionally drop multi-site / reset-heavy tasks.
"""
...
def infer_type(intent: str, site: str | None) -> str:
    s = intent.lower()
    has_price = any(
        k in s
        for k in (
    ...
    has_contact = any(
        k in s for k in ("phone", "email", "contact", "message", "call", "text")
    )
    has_date = any(k in s for k in ("posted", "date", "year", "month", "day"))
    has_location = any(
        k in s
        for k in ("city", "county", "zip", "state", "pennsylvania", "borough", "near")
    )
    has_category = any(k in s for k in ("category", "categories", "section", "filter"))

    if site == "reddit":
        if any(k in s for k in ("top comment", "comments", "comment")):
            return "comments"
        if any(k in s for k in ("post", "submit", "flair", "upvote", "downvote")):
            return "interaction"
    ...
    if site == "shopping":
        if any(k in s for k in ("add to cart", "checkout", "place order", "buy")):
            return "purchase"
        if any(k in s for k in ("review", "rating")):
            return "review"
...
class TaskView:
    ...
    @staticmethod
    def from_task(task: dict[str, Any], site: str | None) -> "TaskView":
...
def dedup_by_intent(tasks: list[TaskView], threshold: float) -> list[TaskView]:
    if threshold <= 0:
        return tasks
    kept: list[TaskView] = []
    for t in sorted(
        tasks, key=lambda x: (x.overall, x.reasoning, x.visual), reverse=True
    ):
...
def pick_with_budget(
    ...
    if len(selected) < budget:
        ...
        for t in remaining:
            if len(selected) >= budget:
                break
            if dedup_threshold > 0 and any(
                jaccard(t.tokens, k.tokens) >= dedup_threshold for k in selected
            ):
                continue
            selected.append(t)
...
def write_markdown(tasks: Iterable[TaskView], out_path: Path) -> None:
    ...
    for t in tasks:
        reset = 1 if t.task.get("require_reset") else 0
        start_url = str(t.task.get("start_url", ""))
        lines.append(
            f"| {t.task_id} | {t.template_id} | {t.ttype} | {reset} | {start_url} | {t.intent} |"
        )
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    ap.add_argument("--site", choices=["classifieds", "reddit", "shopping", "generic"])
    ap.add_argument("--per-template", type=int, default=1)
    ap.add_argument(
        "--only-site-start-url",
        action="store_true",
        help="Keep only tasks whose start_url does not include '|AND|' and contains the site token (e.g. __REDDIT__).",
    )
    ap.add_argument(
        "--exclude-reset",
        action="store_true",
    ...
    ap.add_argument(
        ...
        action="append",
        default=[],
        help="Explicit task_id to exclude (repeatable).",
    )
    args = ap.parse_args()
    ...
    exclude_ids = {int(x) for x in args.exclude_task_id if str(x).isdigit()}
    site = None if args.site == "generic" else args.site
    views = [TaskView.from_task(t, site=site) for t in raw if t.get("task_id") not in exclude_ids]

    if args.only_site_start_url and site is not None:
        token = f"__{site.upper()}__"
        views = [
            v
            for v in views
            if "|AND|" not in str(v.task.get("start_url", ""))
            and token in str(v.task.get("start_url", ""))
        ]

    if args.exclude_reset:
        views = [v for v in views if not bool(v.task.get("require_reset"))]

    stage1 = pick_per_template(
        views, per_template=max(1, args.per_template), seed=args.seed
    )
    stage2 = pick_with_budget(
        stage1,
        budget=max(0, args.budget),
        seed=args.seed,
        dedup_threshold=max(0.0, args.dedup_threshold),
    )

    stage2.sort(
        key=lambda x: (str(x.ttype), -x.overall, int(x.task_id) if str(x.task_id).isdigit() else 10**9)
    )

    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(
        json.dumps([t.task for t in stage2], ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    write_markdown(stage2, md)

    templates = Counter(v.template_id for v in views)
    print(
        f"input={inp} tasks={len(raw)} templates={len(templates)} "
        f"stage1={len(stage1)} stage2={len(stage2)} output={out}"
    )
    return 0
